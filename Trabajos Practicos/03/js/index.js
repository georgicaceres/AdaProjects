...
function chargeBar() {
  ...
  function frame() {
    if (width >= 100) {
      ...
      $('#dialogBox').addClass('animated zoomIn');
...
// Set number of moves depending on level choose.
function setMoves(levelValue) {
  if (levelValue == 0){
    chances = 18;
...
function cardClick(item) {
  ...
  if (arraySelected.length == 2) {
    ...
    if (arraySelected[0] == arraySelected[1]) {
      ...
      item.show("scale", {percent: 200}, "slow");
      $('#'+cardSelected[0]).removeClass('click');
      $('#'+cardSelected[0]).show("scale", {percent: 200}, "slow");
      item.addClass('animated rotateOut');
      $('#'+cardSelected[0]).addClass('animated rotateOut');
      $('#score').html(score);
      arraySelected = [];
      cardSelected = [];
      if ($('.click').length === 0 ) {
...
function youLoose() {
  ...
  activeToggle($('#close'));
  $('#dialogResult').addClass('animated zoomIn');
...
// Event on close icon
$('.flex-container').on('click', '#close', function() {
  activeToggle($('#close'));
  activeToggle($('#dialogExit'));
  $('#dialogExit').addClass('animated zoomIn');
  activeToggle($('.info'));
  $('#board').css('display', 'none');
})

// Event on Yes Button
$('#quiteYes').on('click', function() {
  activeToggle($('#dialogResult'));
  activeToggle($('#dialogExit'));
  reset();
})
